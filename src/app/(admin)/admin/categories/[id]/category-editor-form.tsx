...
export function CategoryEditorForm({ category, defaultKind = CategoryKind.POST }: CategoryEditorFormProps) {
  ...
  const currentKind = category?.kind ?? defaultKind;
      ...
      <section className="grid gap-6">
        {isCreateMode ? (
          <label className="grid gap-2 border-b border-zinc-200/80 pb-4 dark:border-zinc-800/80">
            ...
            >
              <option value={CategoryKind.POST}>文章分类</option>
              <option value={CategoryKind.UPDATE}>动态分类</option>
            </select>
          </label>
        ) : (
